...
vector<double> transfor_car_coord(double psi, double x_car, double y_car, double x_point, double y_point){
  double car_x = (x_point - x_car) * cos(psi) + (y_point - y_car) * sin(psi);
  double car_y = (y_point - y_car) * cos(psi) - (x_point - x_car) * sin(psi);
  return {car_x, car_y};
}

vector<double> delayed_state(double cte0, double epsi0, double v, double delta, double a, int actuatorDelay){
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          Eigen::VectorXd ptsx_transformed = Eigen::VectorXd(ptsx.size());
          Eigen::VectorXd ptsy_transformed = Eigen::VectorXd(ptsx.size());
          ...
          auto var_d = delayed_state(cte0, epsi0, v, delta, a, actuatorDelay);

          // Define the state vector.
          Eigen::VectorXd state(6);
          state << var_d[0], var_d[1], var_d[2], var_d[3], var_d[4], var_d[5];
